...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	if( Gen::StrStartsWith( $seraph_accel_g_prepCont, 'tmp' ) )
	{
		if( Gen::GetArrField( $settCache, array( 'fastTmpOpt' ), false ) )
		{
			...
			{
				if( !headers_sent() )
					header( 'X-Seraph-Accel-Content: 2.27.26; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
	...
	{
		if( $seraph_accel_g_prepPrms !== null && ($seraph_accel_g_prepPrms[ 'lazyInvTmp' ]??null) && !Gen::GetArrField( $settCache, array( 'fastTmpOpt' ), false )  )
		{
			$bufferTmp = $buffer;
...
function ContentPreProcess( $buffer, $settRc, $test = false )
{
	...
	if( Gen::GetArrField( $settRc, array( 'thmXStr' ), false ) && @preg_match( '@<style\\s[^>]*id=[\'"]xstore-inline-css-inline-css[\'"][^>]*>([^<]*)</style>@', $buffer, $m ) )
	{
		...
		$offs = 0;
		while( preg_match( '@<\\w+\\s[^>]*class=[\'"][^\'"]*((?:slider|menu-list)-)(\\d+)@S', $buffer, $m, PREG_OFFSET_CAPTURE, $offs ) )
		{
			$offs = $m[ 0 ][ 1 ] + strlen( $m[ 0 ][ 0 ] );
		...
		$offs = 0;
		while( preg_match( '@<\\w+\\s[^>]*class=[\'"][^\'"]*((?:menu-item)-)(\\d+)@S', $buffer, $m, PREG_OFFSET_CAPTURE, $offs ) )
		{
			$offs = $m[ 0 ][ 1 ] + strlen( $m[ 0 ][ 0 ] );
		...
		$offs = 0;
		while( preg_match( '@<\\w+\\s[^>]*class=[\'"][^\'"]*(slider-item-)(\\d+)@S', $buffer, $m, PREG_OFFSET_CAPTURE, $offs ) )
		{
			$offs = $m[ 0 ][ 1 ] + strlen( $m[ 0 ][ 0 ] );
